...
export const Smd: React.FC<Props> = ({ data,onDelete, onEdit }) => {
  const [isExpanded, setExpand] = useState(false);

  return (
    <div className={styles.equipment}>
      <div className={styles.gateway_card} onClick={() => setExpand(!isExpanded)}>
        <Svg id="smd-connect" className={`${styles.gateway_card_icon} ${gatewayStatus(data.IsOnline, data.AlarmedDeviceCount > 0)}`} />
        <div className={styles.gateway_card_data}>
          <p className={styles.gateway_card_name}>SMD Connect</p>
        ...
        <div className={styles.controls} style={{ position: "absolute", right: "0%", bottom: "0%" }}>
          <ControlButton icon="edit" onClick={() => onEdit(data.Id, "smd")} />
